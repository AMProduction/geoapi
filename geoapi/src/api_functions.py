...
def get_region_stats(region: str) -> dict:
    ...
    if area or gross_yield or weighted_average_yield_per_hectare:
        return {"total_area": area, "total_yield": gross_yield, "average_yield": weighted_average_yield_per_hectare}


def get_nearby_fields(x: float, y: float, distance: int, crop="") -> dict:
    query_result = db_tools.get_nearby_fields_db(x, y, distance, crop)
    if query_result:
...
def get_fields_inside_parallelogram(x0: float, y0: float, x1: float, y1: float, x2: float, y2: float, x3: float,
                                    y3: float, crop="") -> dict:
    query_result = db_tools.get_fields_inside_parallelogram_db(x0, y0, x1, y1, x2, y2, x3, y3, crop)
    if query_result:
        GeoJSON = create_GeoJSON(query_result)
        return GeoJSON


def get_intersect_fields(geometry: str, crop="") -> dict:
    if db_tools.is_valid(geometry):
        query_result = db_tools.get_intersect_fields_db(geometry, crop)
        if query_result:
            GeoJSON = create_GeoJSON(query_result)
            return GeoJSON
